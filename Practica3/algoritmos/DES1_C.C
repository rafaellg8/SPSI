/* des1_c.c */
/* DES Outerbridge single cipher: 64 bit key, 64 bit block with cipher block chaining */
...
/* under MSDOS, NMAKE /F DES1_C.MAK all clean */
...
crypt_file (char *source, char *dest)
{
    ...
    while (count = fread (buffer, sizeof (char), count, infile))
    {
        if (encrypt_or_decrypt == ENCRYPTION_SELECT)        
        {
            ...
            {
                des ((unsigned char *) cbc, (unsigned char *) seed);
                for (i=0; i<8; i++)
                    buffer[i] ^= seed[i];
            }
            for (i=0; i<8; i++)
        ...
        else
        {
            ...
            else
            {
                ...
                des ((unsigned char *) fcbc, (unsigned char *) seed);
...
/* DES Weak keys */
static unsigned char DES_weak[32] =
{
...
/* DES Semi-weak keys */
static unsigned char DES_semi_weak[96] =
{
...
/* DES Possibly weak keys are not identified */
...
process_key (char *s, char *key)
{
    ...
    if (hex_count == 16 && nonhex_count == 0)
    {
        hex_count = 0;
    ...
    for (i = 0; i < 24; i += 8)
    {
        value = 0;
        for (j = 0; j < 8; j++)
            if (key[j] == DES_weak[(i + j)])
                value++;
        if (value == 8)
            fprintf (stderr, "A weak key has been detected.\n");
    }

    /* warn of semi-weak keys */
    for (i = 0; i < 96; i += 8)
    {
        value = 0;
        for (j = 0; j < 8; j++)
            if (key[j] == DES_semi_weak[(i + j)])
                value++;
        if (value == 8)
            fprintf (stderr, "A semi-weak key has been detected.\n");
    }
