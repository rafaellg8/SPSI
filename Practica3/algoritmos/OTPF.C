...
/* under MSDOS, NMAKE /F OTPF.MAK all clean */
...
crypt_file (char *source, char *dest)
{
    ...
    */
    printf ("%s %s %ld %ld\n", dest, otp_name, offset, ftell (otpfile));
    offset = ftell (otpfile);
...
/* strictly syntax check:  name and number */
...
validate_string (char *s)                        /* get otp_name and offset */
{
    ...
    if (fseek (otpfile, offset, SEEK_SET))      /* Error setting offset to OTP file */
        return 0;
