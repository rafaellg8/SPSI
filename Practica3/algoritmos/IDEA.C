 ...
 *
 *      Integration to CRYPT.C Copyright 1994 Willis E. Howard, III.
 *      Problems with main() fixed.  Code for burn() and fill0() was
 *      written.  INDENT was run with my favorite settings.
 *
...
/* Copy and destroy temp copy */
    ...
    for (i=0; i<IDEAKEYLEN; i++)                 /* burn (temp) */
        temp[i] = 0;
}                                                /* ideaInvertKey */
...
/*
 *  Using Microsoft C 6.0 and the command line:  cl /DTEST /G3 idea.c
 *  this module performed at 1819 kbps on a Gateway 2000 4DX2-66V
 *  but only at 330 kbps with compiler option /G0 for 8086 mode.
 */
...
ideaCfbReinit (struct IdeaCfbContext * context, byte const * iv)
{
    ...
    if (iv)
        ...
        for (i=0; i<8; i++)         /* fill0 (context->iv, 8) */
            context->iv[i] = '\0';
